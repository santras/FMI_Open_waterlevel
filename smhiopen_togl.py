...
def open_swedenfiles(file,header):
    ...
    if ((data[6].split(";"))[0])=="Datum Tid (UTC)":    # juts an extra check that header size is ok
        ...
        #print(minutes[0:10])
        date_time=[]
        for inde in range(len(minutes)):
            # changes date + time strings into datetime object
            # datetime allows for example - and + operations and sorting by date and time
...
def check_data(name,sl_variables):
    ...
    for index in range(len(sl_variables)):              # Missing= either 9 as a quality flag or nan as the value
        if sl_variables[index][2]=="9":
            ...
            nans=nans+1
            missing = missing + 1
        elif ((sl_variables[index][2])=="o") or ((sl_variables[index][2])=="O") :    # Unknown quality flag=3
            sl_variables[index][2]="3"
        elif ((sl_variables[index][2])=="y") or ((sl_variables[index][2])=="Y"):
            sl_variables[index][2]= "7"
            counting_y=counting_y+1
            # Swedish quality flag=Misstänkta eller aggregerade värden. Grovt kontrollerade arkivdata och okontrollerade realtidsdata (senaste 2 tim).
            # Interpeted here as Averaged=7
        elif ((sl_variables[index][2])=="g") or ((sl_variables[index][2])=="G"):    # Good data=1
            sl_variables[index][2] = "1"
...
def main():
    ...
    for file in glob.glob("*.csv"):                 # Opens all that ends with .csv in the path folder one by one
        (sl_variables,header,okey,station)=open_swedenfiles(file,Header_dict)        # Function 3
        ...
        if not okey:
            ...
            exit()
        print(file)
        process_file(file,sl_variables,header,header_order,station) # Function 4
